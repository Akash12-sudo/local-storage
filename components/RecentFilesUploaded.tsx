...
const RecentFilesUploaded = ({ files, currentUser }: { files: Models.Document[], currentUser: Models.Document}) => {
  ...
  return (
    <div className="w-full h-full p-2 rounded-2xl shadow-sm flex flex-col bg-white cursor-pointer hover:animate-pulse duration-200 ">
        ...
        {files.map((file, id) => (
            <div key={id} className="w-full flex justify-center items-start m-2 gap-x-4">
                <Thumbnail
                type={file.type}
                ...
                <div className="w-full h-full flex flex-row items-start justify-between gap-x-4 antialiased mt-2">
                    <div className="w-full flex flex-col">
                        <p className="w-1/2 text-sm font-medium line-clamp-1 text-wrap">{file.name}</p>
                        <p className="w-1/2 text-slate-400">{formatDateTime(file.$createdAt)}</p>
                    </div>
